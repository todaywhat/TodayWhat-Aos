...
class MealWidgetProvider : AppWidgetProvider() {
    ...
    override fun onUpdate(
        ...
        val widget = ComponentName(context, MealWidgetProvider::class.java)

        appWidgetIds.forEach { _ ->
            val serviceIntent = Intent(context, MealWidgetRemoteViewsService::class.java)
            val views = RemoteViews(context.packageName, R.layout.meal_widget_small)
            views.setRemoteAdapter(R.id.mealSmallList, serviceIntent)
